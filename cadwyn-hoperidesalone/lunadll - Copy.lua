...
function Lejes:onLoop()
	--Text.print(player.speedX,100,100)
...
function onLoopSection10()
	...
	if (boss == true) then
		...
		for  k,v in pairs(NPC.get(77, 10))  do
			...
			if (freezetime == 0 and frozen == false) then
				freezecounter = 450
				frozen = true
				freezetime = 600
				bosscooldown = 300
				Lejes.firecooldown2 = 250
	...
	if (Lejes.firecooldown2 <= 0) then
		for  k,v in pairs(NPC.get(77, 10))  do
			...
			Lejes.fireballnpc = spawnNPC(282, Lejes.fireballx, Lejes.firebally, player.section);
			Lejes.fireballnpc2 = spawnNPC(269, Lejes.fireballx, Lejes.firebally, player.section);
			Lejes.fireballnpc.speedX = Lejes.fireballspeedX;
			Lejes.fireballnpc2.speedY = 11;
			Lejes.fireballnpc:mem(0xEC, FIELD_FLOAT, Lejes.fireballdir);
			Lejes.fireballnpc:mem(0x118, FIELD_FLOAT, Lejes.fireballdir);
...
function WinJingle()
	...
	Audio.MusicOpen("mm10nsf.nsf|19")
	Audio.MusicPlay()
	cantshoot = true
end
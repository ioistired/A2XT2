...
function multipoints:onCollected(id)
	 rc.currency:save();
	  ...
	  settings.TextSettings:set("CurrentLevel", tostring(mem(0x00B250B0, FIELD_STRING)));
...
function rc.onStart()
	if(not isOverworld) then
		...
		if(tostring(mem(0x00B250B0, FIELD_STRING)) ~= settings.TextSettings:get("CurrentLevel")) then
